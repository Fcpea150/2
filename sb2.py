...
import time, random, sys, json, codecs, threading, glob, re, string, os, requests, subprocess, six, ast, pafy, pytz, urllib.request, urllib.parse, urllib.error, urllib.parse,antolib,subprocess,unicodedata,GACSender
...
line = LINE()
#line = LINE("เมล","พาส")
#line = LINE('EymAGOMHpGkNTb3zgLzb.WaU1s5FRS0mj6Vz/f6B1cW.4lIkFHNbCuGE/N0oMX44HWpgjYuI+WcZaZqd9iltze4=')
...
#==============================================================================#
                ...
                if text.lower() == 'help':
                    myHelp = myhelp()
                    line.sendReplyMessage(msg.id, msg.to, str(myHelp))
                    ...
                    myHelp1 = myhelp1()
                    line.sendReplyMessage(msg.id, msg.to, str(myHelp1))
                    sendMessageWithMention(to, lineMID)
                elif text.lower() == 'help2':
                    helpSet = helpset()
                    line.sendReplyMessage(msg.id, msg.to, str(helpSet))
                    sendMessageWithMention(to, lineMID)
                elif text.lower() == 'help3':
                    listGrup = listgrup()
                    line.sendReplyMessage(msg.id, msg.to, str(listGrup))
                elif text.lower() == 'help4':
                    helpSetting = helpsetting()
                    line.sendReplyMessage(msg.id, msg.to, str(helpSetting))
                elif text.lower() == 'help5':
                    socMedia = socmedia()
                    line.sendReplyMessage(msg.id, msg.to, str(socMedia))
                elif text.lower() == 'help6':
                    helpTextToSpeech = helptexttospeech()
                    line.sendReplyMessage(msg.id, msg.to, str(helpTextToSpeech))
                elif text.lower() == 'help7':
                    helpLanguange = helplanguange()
                    line.sendReplyMessage(msg.id, msg.to, str(helpLanguange))
                elif text.lower() == 'helpsiri':
                    helpSiri= helpsiri()
                    line.sendReplyMessage(msg.id, msg.to, str(helpSiri))
                    sendMessageWithMention(to, lineMID)
#==============================================================================#
                    ...
                    line.sendReplyMessage(msg.id, msg.to, "[ %s Seconds ] [ " % (elapsed_time) + str(int(round((time.time() - start) * 1000)))+" ms ]")
                    #print (to,"Speed sender in group {}").format(puy.getGroup(to).name)
                elif msg.text in [".speed",".sp",".สปีด",".Speed",".Sp"]:
                    start = time.time()
                    line.sendReplyMessage(msg.id, msg.to, "การตอบสนองของบอท(。-`ω´-)")
                    elapsed_time = time.time() - start
                    line.sendReplyMessage(msg.id, msg.to, "[ %s Seconds ]\n[ " % (elapsed_time) + str(int(round((time.time() - start) * 1000)))+" ms ]")
                    ...
                    line.sendReplyMessage(msg.id, msg.to, "กำลังเริ่มต้นใหม่ ... โปรดรอสักครู่ ..")
#                    line.sendMessage(to, "Success Restarting.")
                    ...
                    line.sendReplyMessage(msg.id, msg.to, "ระยะเวลาการทำงานของบอท(。-`ω´-)\n{}".format(str(runtime)))
                elif text.lower() == '.เวลออน':
                    timeNow = time.time()
                    runtime = timeNow - botStart
                    runtime = format_timespan(runtime)
                    line.sendReplyMessage(msg.id, msg.to, "ระยะเวลาบอททำงาน {}".format(str(runtime)))
                    ...
                    try:
                        ...
                        line.sendReplyMessage(msg.id, msg.to, str(ret_))
                        line.sendMentionFooter(to, '「ผู้สร้างบอท」\n', sender, "https://line.me/ti/p/~tomebot333", "http://dl.profile.line-cdn.net/"+line.getContact(sender).pictureStatus, line.getContact(sender).displayName);line.sendMessage(to, line.getContact(sender).displayName, contentMetadata = {'previewUrl': 'http://dl.profile.line-cdn.net/'+line.getContact(sender).pictureStatus, 'i-installUrl': 'https://line.me/ti/p/~samuri5.', 'type': 'mt', 'subText': "WEE SELFBOT", 'a-installUrl': 'https://line.me/ti/p/~samuri5.', 'a-installUrl': ' https://line.me/ti/p/~samuri5.', 'a-packageName': 'com.spotify.music', 'countryCode': 'ID', 'a-linkUri': 'https://line.me/ti/p/~samuri5.', 'i-linkUri': 'https://line.me/ti/p/~samuri5.', 'id': 'mt000000000a6b79f9', 'text': 'Khie', 'linkUri': 'https://line.me/ti/p/~samuri5'}, contentType=19)
                    except Exception as e:
                        line.sendReplyMessage(msg.id, msg.to, str(e))
                elif text.lower() == '!ข้อมูล':
                    try:
                        arr = []
                        owner = "uc00bb54b97ebb34309eaf67e17a6f50b"
...
#==============================================================================#
                    ...
                    line.sendReplyMessage(msg.id, msg.to,"[👇ชื่อของพี่👇]")
                    sendMessageWithMention(to, lineMID)
                    line.sendReplyMessage(msg.id, msg.to,"[สเตตัส]\n" + me.statusMessage)
                    line.sendContact(to, lineMID)
                    line.sendImageWithURL(msg.to,"http://dl.profile.line-cdn.net/" + me.pictureStatus)
                    cover = line.getProfileCoverURL(lineMID)
                    line.sendImageWithURL(msg.to, cover)
                    line.sendReplyMessage(msg.id, msg.to,str(settings["comment"]))
                ...
                if msg.text in ["ไอดี","Mid","mid","MID","มิด"]:
                    ...
                    line.sendReplyMessage(msg.id, msg.to, str(settings["welcome"]))
                elif text.lower() == '.ทักออก':
                    line.sendReplyMessage(msg.id, msg.to, str(settings["bye"]))
                elif text.lower() == '.ทักเตะ':
                    line.sendReplyMessage(msg.id, msg.to, str(settings["kick"]))
                elif text.lower() == '.ข้อความแอด':
                    line.sendReplyMessage(msg.id, msg.to,str(settings["message"]))
                elif text.lower() == '.ข้อความแทค':
                    line.sendReplyMessage(msg.id, msg.to, str(settings["Respontag"]))
                    ...
                    if targets == []:
                        line.sendReplyMessage(msg.id, msg.to, "ม่มีคนใส่ร่องหนในกลุ่มนี้😂")
                    else:
                        mc = ""
                        for target in targets:
                            mc += sendMessageWithMention(to,target) + "\n"
                        line.sendReplyMessage(msg.id, msg.to, mc)
                    ...
                    for g in gs.members:
                        if g.displayName in "":
                            lists.append(g.mid)
                    if lists == []:
                        line.sendReplyMessage(msg.id, msg.to, "ไม่มีmidคนใส่ร่องหน🤗")
                    else:
                        mc = ""
                        for mi_d in lists:
                            mc += "->" + mi_d + "\n"
                    ...
                    for g in gs.members:
                        if g.displayName in "":
                            lists.append(g.mid)
                    if lists == []:
                        line.sendReplyMessage(msg.id, msg.to, "ไม่มีคนใส่ร่องหนในกลุ่มนี้😂")
                    else:
                        for ls in lists:
                    ...
                    for target in targets:
                        try:
                            ...
                            line.sendReplyMessage(msg.id, msg.to, "Success...")
                        #---------------------------------------
                        ...
                        #---------------------------------------
                            ...
                            line.sendReplyMessage(msg.id, msg.to, "Failed!")
                            print (e)
                    ...
                    if msg._from in admin:
                            try:
                                ...
                                line.sendReplyMessage(msg.id, msg.to, "คืนได้แค่ชื่อกับตัสนะ😂😂")

                            except Exception as e:
                                line.sendReplyMessage(msg.id, msg.to,"การคืนร่างล้มเหลว!")
                                print (e)
...
#==============================================================================#
                    ...
                    for target in targets:
                        try:
                            ...
                            line.sendReplyMessage(msg.id, msg.to,"Mimic has been added as")
                            break
                        except:
                            line.sendReplyMessage(msg.id, msg.to,"Added Target Fail !")
                            break
                    ...
                    for target in targets:
                        try:
                            ...
                            line.sendReplyMessage(msg.id, msg.to,"Mimic deleting succes...")
                            break
                        except:
                            line.sendReplyMessage(msg.id, msg.to,"Deleted Target Fail !")
                            break
                elif text.lower() == 'mimiclist':
                    if settings["mimic"]["target"] == {}:
                        line.sendReplyMessage(msg.id, msg.to,"Tidak Ada Target")
                    else:
                        mc = "╔══[ Mimic List ]"
                    ...
                    if mic == "on":
                        if settings["mimic"]["status"] == False:
                            ...
                            line.sendReplyMessage(msg.id, msg.to,"Mimic enabled.")
                    elif mic == "off":
                        if settings["mimic"]["status"] == True:
                            settings["mimic"]["status"] = False
                            line.sendReplyMessage(msg.id, msg.to,"Mimic disabled.")
                    ...
                    if txt[1] == "on":
                        if jmlh <= 100000:
                           for x in range(jmlh):
                               line.sendReplyMessage(msg.id, msg.to, teks)
                        else:
                           line.sendReplyMessage(msg.id, msg.to, "Out of Range!")
                    elif txt[1] == "off":
                        if jmlh <= 100000:
                            line.sendReplyMessage(msg.id, msg.to, tulisan)
                        else:
                            line.sendReplyMessage(msg.id, msg.to, "Out Of Range!")
#==============================================================================#
                elif text.lower() == '.bye':
                          if msg._from in admin:
                              ...
                              line.sendReplyMessage(msg.id, msg.to, "Bye-bye "+str(G.name))
                              line.leaveGroup(msg.to)
                elif text.lower() == '.เชคแอด':
                    group = line.getGroup(to)
                    GS = group.creator.mid
                    line.sendReplyMessage(msg_id,to, None, contentMetadata={'mid':GS}, contentType=13)
                elif text.lower() == '.แอด':
                    group = line.getGroup(to)
                    GS = group.creator.mid
                    line.sendContact(to, GS)
                    line.sendReplyMessage(msg.id, msg.to, "☝คนนี้แหล่ะคนสร้างกลุ่มนี้")
                    ...
                    if msg.toType == 2:
                        ...
                        if group.preventedJoinByTicket == False:
                            ...
                            line.sendReplyMessage(msg.id, msg.to, "「 Group 」\n• https://line.me/R/ti/g/{}".format(str(ticket)))
                        else:
                            line.sendReplyMessage(msg.id, msg.to, "กรุณาเปิดลิ้งกลุ่มก่อนลงคำสั่งนี้ด้วยครับ(。-`ω´-)".format(str(settings["keyCommand"])))

                elif text.lower() == '.เปิดลิ้ง':
                    if msg.toType == 2:
                        group = line.getGroup(to)
                        if group.preventedJoinByTicket == False:
                            line.sendReplyMessage(msg.id, msg.to, "ลิ้งกลุ่มเปิดอยู่แล้ว(。-`ω´-)")
                        else:
                            group.preventedJoinByTicket = False
                            line.updateGroup(group)
                            line.sendReplyMessage(msg.id, msg.to, "เปิดลิ้งกลุ่มเรียบร้อย(。-`ω´-)")

                elif text.lower() == '.ปิดลิ้ง':
                    if msg.toType == 2:
                        group = line.getGroup(to)
                        if group.preventedJoinByTicket == True:
                            line.sendReplyMessage(msg.id, msg.to, "ลิ้งกลุ่มปิดอยู่แล้ว(。-`ω´-)")
                        else:
                            group.preventedJoinByTicket = True
                            line.updateGroup(group)
                            line.sendReplyMessage(msg.id, msg.to, "ลิ้งกลุ่มปิดเรียบร้อย(。-`ω´-)")
...
#==============================================================================#
                    ...
                    if msg.toType == 1:
                        ...
                        members = [mem.mid for mem in room.contacts]
                          ...
                          return line.sendReplyMessage(msg.id, msg.to, '「 TAG 」 • ERROR')
                    if members:
...
#                               aditmadzs.sendMessage(msg.to,"KI\n"+ki.authToken)
                ...
                elif msg.text.lower().startswith(".mp4 "):
                            try:
                                sep = msg.text.split(" ")
                ...
                elif msg.text.lower().startswith(".เพลงmp4 "):
                            try:
                                sep = msg.text.split(" ")
                                textToSearch = msg.text.replace(sep[0] + "เพลงmp4 ","")
                                query = urllib.parse.quote(textToSearch)
                                search_url="https://www.youtube.com/results?search_query="
                                mozhdr = {'User-Agent': 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3'}
                                sb_url = search_url + query
                    ...
                    settings["kickMention"] = True
                    line.sendReplyMessage(msg.id, msg.to,"เปิดระบบเตะคนแท็ก")
                
                elif msg.text in [".ปิดแทคเจ็บ"]:
                    settings["kickMention"] = False
                    line.sendReplyMessage(msg.id, msg.to,"ปิดระบบเตะคนแท็ก")
                    
                elif msg.text in [".เปิดแทค","Tag on"]:
                        settings['detectMention'] = True
                        line.sendReplyMessage(msg.id, msg.to,"เปิดแทคเรียบร้อยแล้ว.")
                
                elif msg.text in [".ปิดแทค","Tag off"]:
                        settings['detectMention'] = False
                        line.sendReplyMessage(msg.id, msg.to,"ปิดแทคเรียบร้อยแล้ว.")

                elif msg.text in [".เปิดแทค2"]:
                    settings["potoMention"] = True
                    line.sendReplyMessage(msg.id, msg.to,"เปิดแทครูปเรียบร้อยแล้วd.")
                
                elif msg.text in [".ปิดแทค2"]:
                    settings["potoMention"] = False
                    line.sendReplyMessage(msg.id, msg.to,"ปิดแทครูปเรียบร้อยแล้ว.")
                    
                elif msg.text in [".เปิดแทค3"]:
                    settings["delayMention"] = True
                    line.sendReplyMessage(msg.id, msg.to,"เปิดระบบแทคกลับคนแทค(○ﾟεﾟ○)")
                
                elif msg.text in [".ปิดแทค3"]:
                    settings["delayMention"] = False
                    line.sendReplyMessage(msg.id, msg.to,"ปิดระบบแทคกลับคนแทค(ˉ(∞)ˉ)")
                elif msg.text.lower() == ".เปิดแทคแชท":
                    settings["detectMentionPM"] = True
                    line.sendReplyMessage(msg.id, msg.to,"เปิดแทคแชทเรียบร้อยครับ")
                elif msg.text.lower() == ".ปิดแทคแชท":
                    settings["detectMentionPM"] = False
                    line.sendReplyMessage(msg.id, msg.to,"ปิดแทคแชทเรียบร้อยครับ")
                elif msg.text.lower().startswith(".ตั้งแทคแชท: "):
                    text = msg.text.lower().replace(".ตั้งแทคแชท: ","")
                    settings["pmMessage"] = text
                    line.sendReplyMessage(msg.id, msg.to, "คำแทคแชท สต คือ : {}".format(str(settings["pmMessage"])))
                elif msg.text.lower().startswith("setrespongroup: "):
                    text = msg.text.lower().replace("setrespongroup: ","")
                    settings["respMessage"] = text
                    line.sendReplyMessage(msg.id, msg.to, "Success Update Response Group to : {}".format(str(settings["respMessage"])))
                    
                elif msg.text in [".เปิดตรวจสอบ"]:
                    settings["Aip"] = True
                    line.sendReplyMessage(msg.id, msg.to,"เปิดระบบตรวจสอบคำหยาบกับบอทบิน  ^ω^")
                
                elif msg.text in [".ปิดตรวจสอบ"]:
                    settings["Aip"] = False
                    line.sendReplyMessage(msg.id, msg.to,"ปิดระบบตรวจสอบคำหยาบกับบอทบินแล้วʕ•ﻌ•ʔ")
                    
                elif msg.text in [".เปิดพูด"]:
                    settings["Api"] = True
                    line.sendReplyMessage(msg.id, msg.to,"เปิดระบบApiข้อความ")
                
                elif msg.text in [".ปิดพูด"]:
                    settings["Api"] = False
                    line.sendReplyMessage(msg.id, msg.to,"ปิดระบบApiข้อความแล้ว")
                    
                elif '.ตั้งแอด: ' in msg.text:
                  if msg._from in admin:
                     spl = msg.text.replace('.ตั้งแอด: ','')
                     if spl in [""," ","\n",None]:
                         line.sendReplyMessage(msg.id, msg.to, "ตั้งข้อความเรืยบร้อย")
                     else:
                         settings["message"] = spl
                         line.sendReplyMessage(msg.id, msg.to, "™❍✯͜͡WEE™SELFBOT✯͜͡❂➣\n👇ตั้งข้อความตอบโต้เมื่อมีคนแอดแล้ว ดังนี้👇\n\n👉{}".format(str(spl)))
                  ...
                  if msg._from in admin:
                     ...
                     if spl in [""," ","\n",None]:
                         line.sendReplyMessage(msg.id, msg.to, "ตั้งข้อความคนคนลบสมาชิดเรียบร้อย")
                     else:
                          settings["kick"] = spl
                  ...
                  if msg._from in admin:
                     ...
                     if spl in [""," ","\n",None]:
                         line.sendReplyMessage(msg.id, msg.to, "ตั้งข้อความคนออกเรียบร้อย")
                     else:
                          settings["bye"] = spl
                  ...
                  if msg._from in admin:
                     ...
                     if spl in [""," ","\n",None]:
                         line.sendReplyMessage(msg.id, msg.to, "ตั้งข้อความคนเข้าเรียบร้อยแล้ว")
                     else:
                          settings["welcome"] = spl
                          line.sendReplyMessage(msg.id, msg.to, "™❍✯͜͡WEE™SELFBOT✯͜͡❂➣\nตั้งค่าข้อความเมื่อมีคนเข้ากลุ่มแล้ว\nดังนี้👇\n\n👉{}".format(str(spl)))
                elif msg.text.lower() == "เปิดกันติ๊ก":
                        settings["sticker"] = True
                        line.sendReplyMessage(msg.id, msg.to,"เปิดเตะคนรันสติกเกอรแล้ว")
                elif msg.text.lower() == "ปิดกันติ๊ก":
                        settings["sticker"] = False
                        line.sendReplyMessage(msg.id, msg.to,"ปิดเตะคนรันสติกเกอรแล้ว")
                    ...
                    line.sendReplyMessage(msg.id, msg.to, "ส่งสติกเกอรที่จะตั้งลงมา")
                elif msg.text.lower() == ".ลบติ๊กคนออก":
                    settings["messageSticker"]["listSticker"]["leaveSticker"] = None
                    line.sendReplyMessage(msg.id, msg.to, "ลบสติกเกอรคนเข้าแล้ว")
                    ...
                    line.sendReplyMessage(msg.id, msg.to,"ลบสติกเกอรคนเตะแล้ว")
                    ...
                    line.sendReplyMessage(msg.id, msg.to, "ส่งสติกเกอรที่จะใช่ลงมา")
                elif msg.text.lower() == ".ลบติ๊กคนแอด":
                    settings["messageSticker"]["listSticker"]["addSticker"] = None
                    line.sendReplyMessage(msg.id, msg.to, "ลบสติกเกอรคนแทคแล้ว")
                    ...
                    line.sendReplyMessage(msg.id, msg.to, "ส่งสติ๊กเกอรที่จะใช่ลงมา")
                elif msg.text.lower() == ".ลบติ๊กคนแอบ":
                    settings["messageSticker"]["listSticker"]["readerSticker"] = None
                    line.sendReplyMessage(msg.id, msg.to,"ลบสติ๊กเกอรคนแอบอ่านแล้ว")
                    ...
                    line.sendMessage(msg.to, t1 + txt + t2)						
                        ...
                        line.sendReplyMessage(msg.id, msg.to,"send a contact to invite user")                            
...
#=============COMMAND KICKER===========================#
                  ...
                  if msg._from in admin: 
                    ...
                    line.sendReplyMessage(msg.id, msg.to,"กรุณาส่งคอทแทค")
                  ...
                  if msg._from in admin: 
                    ...
                    settings["blacklist"] = {}
                    line.sendReplyMessage(msg.id, msg.to,"ทำการลบัญชีดำทั้งหมดเรียร้อย")
                    print ("Clear Ban")
                elif msg.text.lower() == ".คทดำ":
                    if msg._from in lineMID:
                        if settings["blacklist"] == []:
                            line.sendReplyMessage(msg.id, msg.to, "Nothing boss")
                        else:
                       ...
                       for target in targets:
                           try:
                               ...
                               line.sendReplyMessage(msg.id, msg.to,"Limit kaka 😫")
                       ...
                       for target in targets:
                           try:
                               ...
                               line.sendReplyMessage(msg.id, msg.to,"Type👉 Limit Invite")
                    ...
                    if msg._from in admin:
                           ...
                           line.sendReplyMessage(msg.id, msg.to, "เปลี่ยนชื่อเป็น(。-`ω´-) " + string)
                	...
                	if msg._from in admin:
                           ...
                           line.updateProfile(profile_A)
                           line.sendReplyMessage(msg.id, msg.to, "เปลี่ยนตัสเสร็จสิ้น(。-`ω´-)  " + string)
...
#==============FINNISHING PROTECT========================#
                        ...
                        if settings["Wc"] == True:
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"เปิดข้อความต้อนรับเมื่อมีสมาชิกเข้ากลุ่ม   ")
                        else:
                            settings["Wc"] = True
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"เปิดข้อความต้อนรับเมื่อมีสมาชิกเข้ากลุ่ม   ")
                        ...
                        if settings["Wc"] == False:
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"ปิดข้อความต้อนรับเมื่อมีสมาชิกเข้ากลุ่ม   ")
                        else:
                            settings["Wc"] = False
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"ปิดข้อความต้อนรับเมื่อมีสมาชิกเข้ากลุ่ม   ")
                        ...
                        if settings["Nk"] == True:
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"เปิดข้อความแจ้งเตือนเมื่อมีคนลบสมาชิกในกลุ่ม...")
                        else:
                            settings["Nk"] = True
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"เปิดข้อความแจ้งเตือนเมื่อมีคนลบสมาชิกในกลุ่ม...")
                        ...
                        if settings["Nk"] == False:
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"ปิดข้อความแจ้งเตือนเมื่อมีคนลบสมาชิกในกลุ่มแล้ว..")
                        else:
                            settings["Nk"] = False
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"เปิดข้อความแจ้งเตือนเมื่อมีคนลบสมาชิกในกลุ่มแล้ว...")

                elif msg.text.lower() == '.เปิดส่งแขก':
                        if settings["Lv"] == True:
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"เปิดข้อความอำลาเมื่อมีสมาชิกออกกลุ่ม   ")
                        else:
                            settings["Lv"] = True
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"เปิดข้อความอำลาเมื่อมีสมาชิกออกกลุ่ม   ")
                        ...
                        if settings["Lv"] == False:
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"ปิดข้อความอำลาเมื่อมีสมาชิกออกกลุ่ม   ")
                        else:
                            settings["Lv"] = False
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"ปิดข้อความอำลาเมื่อมีสมาชิกออกกลุ่ม   ")
                        ...
                        if settings["checkContact"] == True:
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"เปิดระบบอ่านข้อมูลด้วยคอนแทคไว้อยู่แล้ว ")
                        else:
                            settings["checkContact"] = True
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"เปิดระบบอ่านข้อมูลด้วยคอนแทค")
                elif msg.text.lower() == '.ปิดคท':
                        if settings["checkContact"] == False:
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"ปิดระบบอ่านข้อมูลด้วยคอนแทคไว้อยู่แล้ว ")
                        else:
                            settings["checkContact"] = False
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"ปิดระบบอ่านข้อมูลด้วยคอนแทค")
                elif msg.text.lower() == '.เปิดเช็คโพส':
                        if settings["checkPost"] == True:
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"เปิดระบบเช็คโพสบนทามไลน์ไว้อยู่แล้ว " )
                        else:
                            settings["checkPost"] = True
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"เปิดระบบเช็คโพสบนทามไลน์ ")
                elif msg.text.lower() == '.ปิดเช็คโพส':
                        if settings["checkPost"] == False:
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"ปิดระบบเช็คโพสบนทามไลน์ไว้อยู่แล้ว ")
                        else:
                            settings["checkPost"] = False
                            if settings["lang"] == "JP":
                                line.sendReplyMessage(msg.id, msg.to,"ปิดระบบเช็คโพสบนทามไลน์")
                elif text.lower() == ".แปลงโฉม":
                    settings["changePictureProfile"] = True
                    line.sendReplyMessage(msg.id, msg.to, "ส่งรูปภาพลงมาได้เลยครับผม")
                elif text.lower() == ".เปลี่ยนรูปกลุ่ม":
                    if msg.toType == 2:
                        if to not in settings["changeGroupPicture"]:
                            settings["changeGroupPicture"].append(to)
                        line.sendReplyMessage(msg.id, msg.to,"ส่งรูปภาพลงมาไดเเลยครับผม")
                    ...
                    elapsed_time = time.time() - start
                    line.sendReplyMessage(msg.id, msg.to, "ลบรันเสร็จแล้วขอรับ")
                    line.sendReplyMessage(msg.id, msg.to, "ระยะเวลาที่ใช้: %sวินาที" % (elapsed_time))
                  ...
                  if msg._from in Family:
                      if msg.toType == 2:
                           ...
                           gs = line.getGroup(msg.to)
                           line.sendReplyMessage(msg.id, msg.to,"แบนสมาชิกทุกคนในห้องนี้แล้ว＼（○＾ω＾○）／")
                           targets = []
                           for g in gs.members:
                               if _name in g.displayName:
                                    targets.append(g.mid)
                           if targets == []:
                                line.sendReplyMessage(msg.id, msg.to,"Maaf")
                           else:
                               for target in targets:
                                   if not target in Family:
                                       try:
                                           ...
                                           line.sendReplyMessage(msg.id, msg.to,"พบข้อผิดพลาดที่ไม่ทราบสาเหตุ")
                       ...
                       for target in targets:
                           try:
                               ...
                               line.sendReplyMessage(msg.id, msg.to,"Contact Not Found")
                       ...
                       for target in targets:
                           try:
                               ...
                               f=codecs.open('st2__b.json','w','utf-8')
                               json.dump(settings["blacklist"], f, sort_keys=True, indent=4,ensure_ascii=False)
                               line.sendReplyMessage(msg.id, msg.to,"Succes unban from the blacklist. ")
                               print ("Unbanned User")
                           except:
                               line.sendReplyMessage(msg.id, msg.to,"Contact Not Found")
                  ...
                  if msg._from in Family:
                    if settings["blacklist"] == {}:
                        line.sendReplyMessage(msg.id, msg.to,"ไม่พบ") 
                    else:
                        line.sendReplyMessage(msg.id, msg.to,"รายชื่อผู้ติดดำ")
                        mc = "Blacklist User\n"
                    ...
                    with requests.session() as s:
                        ...
                        line.sendReplyMessage(msg.id, msg.to, str(y))
                ...
                if settings["checkContact"] == True:
                    try:
                        ...
                        line.sendReplyMessage(msg.id, msg.to, "เกิดข้อผิดพลาดในการสำรวจ")
                for image in images:
                    if text.lower() == image:
